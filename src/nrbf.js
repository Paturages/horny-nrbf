...
export const readNrbf = buffer => {
  ...
  const readClassWithMembersAndTypes = (isSystem) => {
    ...
    classObj.members.forEach(member => {
      switch (member.type) {
        case 'SystemClass': member.typeName = readString();
        break;
        case 'Class':
        ...
        break;
        case 'Primitive':
        case 'PrimitiveArray': member.typeName = getPrimitiveTypeEnum(readByte());
        break;
        default: throw new Error(member.type + ' not handled');
      }
    });
    if (!isSystem) {
    ...
    classObj.members.forEach(member => {
      if (['SystemClass', 'Class', 'StringArray', 'PrimitiveArray'].indexOf(member.type) > -1) {
        const recordTypeEnum = readByte();
        switch (recordTypeEnum) {
          // MemberReference
          case 0x09: member.refId = readInteger();
          break;
          default: throw new Error(member.type + ' <' + recordTypeEnum.toString(16).padStart(2, '0') + '> not handled')
        }
      } else if (member.type === 'Primitive') {
        // Most likely a primitive
        switch (member.typeName) {
          case 'Single': member.value = readFloat32();
          break;
          case 'Int32': member.value = readInteger();
          break;
          default: console.log(member); throw new Error(member.typeName + ' not handled');
        }
  ...
  const readClassWithId = () => {
    ...
    // MemberTypeInfo (references, values, etc...)
    classObj.members.forEach(member => {
      if (['SystemClass', 'StringArray', 'PrimitiveArray'].indexOf(member.type) > -1) {
        const recordTypeEnum = readByte();
        switch (recordTypeEnum) {
          // MemberReference
          case 0x09: member.refId = readInteger();
          break;
          default: throw new Error(member.type + ' <' + recordTypeEnum + '> not handled')
        }
      } else if (member.type === 'Primitive') {
        // Most likely a primitive
        switch (member.typeName) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      switch (recordTypeEnum) {
        ...
        case 0x0d:
          ...
          i += nullCount - 1;
          for (let j = 0; j < nullCount; ++j) objs[objectId].values.push({ value: null });
        break;
        default: throw new Error('ArraySingleString <' + recordTypeEnum + '> not handled')
      }
    }
  ...
  const readBinaryArray = () => {
    ...
    obj.lengths.forEach(length => {
      for (let i = 0; i < length; ++i) {
        ...
        switch (recordTypeEnum) {
          // MemberReference
          case 0x09: obj.values.push({ refId: readInteger() });
          break;
          case 0x0a: obj.values.push({ value: null });
          break;
          // ObjectNullMultiple256
          case 0x0d:
            const nullCount = readByte();
            i += nullCount - 1;
            for (let j = 0; j < nullCount; ++j) obj.values.push({ value: null });
          break;
          default: console.log(obj); throw new Error('BinaryArray <' + recordTypeEnum + '> not handled')
        }
      }
    });
    objs[obj.objectId] = obj;
  ...
  const readArraySinglePrimitive = () => {
    ...
    for (let i = 0; i < obj.length; ++i) {
      switch (obj.primitiveTypeEnum) {
        case 'Single': obj.values.push({ value: readFloat32() });
        break;
        case 'Int32': obj.values.push({ value: readInteger() });
        break;
        default: throw new Error(obj.primitiveTypeEnum + ' not handled')
      }
    }
    objs[obj.objectId] = obj;
  };
  ...
  const compileData = () => {
    ...
    return { name: objs[1].objectName, data, _debug: { metadata, objs } };
  }
  ...
  while (offset < buffer.length) {
    ...
    try {
      switch (recordTypeEnum) {
        case 0x00: readSerializedStreamHeader();
        break;
        case 0x01: readClassWithId();
        break;
        case 0x04:
        case 0x05: readClassWithMembersAndTypes(recordTypeEnum === 0x04);
        break;
        case 0x07: readBinaryArray();
        break;
        case 0x0C: readBinaryLibrary();
        break;
        case 0x0F: readArraySinglePrimitive();
        break;
        case 0x11: readArraySingleString();
        break;
        // MessageEnd
        case 0x0b: return compileData();
        default: console.log('Unrecognized', offset, '0x'+recordTypeEnum.toString(16), metadata, objs);
